...
import { useToast } from "@/hooks/growth/use-toast";
...
export default function Page() {
  ...
  const handleSave = async () => {
    ...
    // Supabase save
    try {
      const { createClient: _createClient } = await import("@/lib/supabase/client");
  ...
  return (
    <div className="min-h-screen bg-background" dir={isHe ? "rtl" : "ltr"}>
      <div className="container mx-auto max-w-lg px-4 py-20">
        <Button variant="ghost" size="sm" onClick={() => router.push("/growth/hub")} className="mb-6 gap-2">
