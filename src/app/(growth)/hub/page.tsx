...
import { getTotalUsers } from "@/lib/growth/socialProofData";
import Header from "@/components/growth/Header";
import ModulePipeline from "@/components/growth/ModulePipeline";
    ...
    <div className="min-h-screen bg-background">
      ...
      {user && (
        <section className="container mx-auto px-4 py-4 max-w-2xl">
          <Card role="button" tabIndex={0} className="border-2 border-amber-500/30 bg-amber-500/5 cursor-pointer hover:shadow-lg transition-all focus:outline-none focus:ring-2 focus:ring-primary" onClick={() => router.push("/growth/" + nextStep.route.replace(/^\//, ""))} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") router.push("/growth/" + nextStep.route.replace(/^\//, "")); }}>
      ...
      <section className="container mx-auto px-4 py-8 max-w-2xl text-center">
        <div className="flex flex-wrap justify-center gap-3">
          {planCount > 0 && (
            <Button variant="outline" size="sm" onClick={() => router.push("/growth/plans")}>
              {isHe ? `תוכניות שמורות (${planCount})` : `Saved Plans (${planCount})`}
            </Button>
          )}
          {user && (
            <Button variant="outline" size="sm" onClick={() => router.push("/growth/dashboard")}>
