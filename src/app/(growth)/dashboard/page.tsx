...
import { generateWeeklyPulse } from "@/lib/engines/growth/pulseEngine";
import { buildUserKnowledgeGraph } from "@/lib/engines/growth/userKnowledgeGraph";
import { calculateHealthScore } from "@/lib/engines/growth/healthScoreEngine";
import { generateFunnel } from "@/lib/engines/growth/funnelEngine";
import { getRecommendedNextStep } from "@/lib/engines/growth/nextStepEngine";
...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <Header onSavedPlans={() => router.push("/growth/plans")} />
      <main className="container mx-auto px-4 pt-24 pb-16 max-w-4xl">
        ...
        <Card className="mb-6 cursor-pointer hover:shadow-lg transition-all border-2 border-amber-500/30 hover:border-amber-500 bg-gradient-to-r from-amber-500/5 to-transparent" onClick={() => router.push("/growth/" + nextStep.route.replace(/^\//, ""))}>
        ...
        <div className="grid gap-3 sm:grid-cols-2 mb-6">
          <Card className="cursor-pointer hover:shadow transition-shadow" onClick={() => router.push("/growth/wizard")}>
          ...
          <Card className="cursor-pointer hover:shadow transition-shadow" onClick={() => router.push("/growth/differentiate")}>
            <CardContent className="p-3 flex items-center gap-3">
        ...
        {lastPlan && (
          <Card className="mb-6">
            ...
            <CardContent className="flex items-center justify-between">
              ...
              <Button size="sm" onClick={() => router.push(`/growth/plans/${lastPlan.id}`)}>
                {isHe ? "המשך →" : "Continue →"}
              </Button>
            </CardContent>
          </Card>
        ...
        {savedPlans.length > 0 && (
          <div className="flex items-center justify-between mb-3">
            ...
            <Button size="sm" variant="ghost" onClick={() => router.push("/growth/plans")}>{isHe ? "הצג הכל" : "View All"}</Button>
          </div>
        )}
        <div className="grid gap-3 sm:grid-cols-2">
          {savedPlans.slice(0, 4).map((plan) => (
            <Card key={plan.id} className="cursor-pointer hover:shadow transition-shadow" onClick={() => router.push(`/growth/plans/${plan.id}`)}>
        ...
        <div className="fixed bottom-20 end-4 z-30">
          <Button size="lg" className="rounded-full h-14 w-14 shadow-lg funnel-gradient border-0" onClick={() => router.push("/growth/wizard")}>
