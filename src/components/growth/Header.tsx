...
import AuthModal from "@/components/growth/AuthModal";
import AchievementBadgesPanel from "@/components/growth/AchievementBadgesPanel";
...
import { useDarkMode } from "@/hooks/growth/useDarkMode";
import ModuleProgressStrip from "@/components/growth/ModuleProgressStrip";
    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-background border-b border-border" style={{ paddingTop: "env(safe-area-inset-top)" }}>
        <div className="container mx-auto flex h-14 items-center justify-between px-4">
          {/* Logo */}
          <div role="button" tabIndex={0} aria-label="FunnelForge Home" className="flex items-center gap-2 cursor-pointer" onClick={() => router.push("/growth/")} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") router.push("/growth/"); }}>
            ...
            {user ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent align={isHe ? "start" : "end"} className="w-48">
                  <DropdownMenuItem onClick={() => router.push("/growth/")}>
                    <Home className="h-4 w-4 me-2" />
                    {isHe ? "מרכז מודולים" : "Module Hub"}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => router.push("/growth/dashboard")}>
                    <Settings className="h-4 w-4 me-2" />
                    {isHe ? "דשבורד" : "Dashboard"}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => router.push("/growth/plans")}>
                  ...
                  <DropdownMenuItem onClick={() => router.push("/growth/profile")}>
